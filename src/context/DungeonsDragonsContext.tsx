import React, { FC, useCallback, useReducer, createContext, useMemo } from 'react';
import { IDungeonsDragonsCTX, IDungeonsDragonsState, ISpellDetails, Spells, TDungeonsDragonsAction } from '../types';
...
const dungeonsDragonsReducer = (state: IDungeonsDragonsState, action: TDungeonsDragonsAction) => {
	switch (action.type) {
		case 'SET_SPELLS': {
			return {
				...state,
		...
		case 'SET_FAVORITE_SPELL': {
			...
			if (favoriteSpells.includes(action.payload)) {
				favoriteSpells = favoriteSpells.filter((item) => item !== action.payload);
				return {
					...state,
					favoriteSpells,
				};
...
const DungeonsDragonsProvider: FC<{ children: JSX.Element }> = ({ children }) => {
